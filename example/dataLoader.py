...
def getAllDataLoaders(nodeIDs, n_normal):
	...
	for i, nodeID in enumerate(nodeIDs):
		if i <= n_normal:
			nodeDataLoaders[nodeID] = torch.utils.data.DataLoader(train_datasets[i], batch_size=10, shuffle=True)
		else:
			nodeDataLoaders[nodeID] = torch.utils.data.DataLoader(fake_datasets[i], batch_size=10, shuffle=True)
